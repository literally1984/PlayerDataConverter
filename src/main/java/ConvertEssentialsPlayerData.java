...
public class ConvertEssentialsPlayerData implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        File WorldFilePlayers = new File(Bukkit.getWorldContainer().getAbsolutePath().substring(0, Bukkit.getWorldContainer().getAbsolutePath().length() - 1) + "plugins/Essentials/userdata/");
        ...
        for (File child: Players) {
            ...
            File RenamePlayer = new File(Bukkit.getWorldContainer().getAbsolutePath().substring(0, Bukkit.getWorldContainer().getAbsolutePath().length() - 1) + "plugins/Essentials/userdata/" + PlayerOfflineUUID.toString() + ".yml");
